...
## [0.4.11-SNAPSHOT]
### 新增
- **`AgentCore.ttsParallel()`**：新增TTS并行播放接口。`tts()` / `ttsSync()` 通过客户端队列串行下发请求，服务端回调后再发起下一个；`ttsParallel()` 绕过客户端队列直接下发，多段文本并发请求由服务端顺序播放，可有效消除连续播报时片段间的停顿

### 更新
- **RobotCore版本升级**：内置 RobotCore 依赖从 `11.3` 升级至 `12.2.1`
