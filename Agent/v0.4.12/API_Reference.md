# AgentOS SDK v0.4.12 API 参考文档

> **版本信息**
> 当前SDK版本: 0.4.12
> 依赖: `implementation 'com.orionstar.agent:sdk:0.4.12-SNAPSHOT'`
> ROM基线: V12.6
...
- [OnAgentStatusChangedListener](#onagentstatuschangedlistener) - Agent状态变化监听接口
...
**回调时机：** 完整播放结束。`status=1`表示播放成功，`status=2`表示播放失败或被打断。
...
```
请求级TTS同步接口，需在协程中调用。用于确认TTS请求是否成功，不等待播放结束。

**参数说明：**
- `text: String` - 要播放的文本
- `timeoutMillis: Long` - 请求超时时间，单位毫秒，默认3秒；不包含语音播放时长

**返回值：**
- `status=1` - TTS请求成功
- `status=2` - 请求超时、失败或被打断
- `result` - 当前为`null`

**注意：**
- `status=2`不区分具体失败原因
- `text`为空字符串时直接返回`status=2`
- 网络状况较差时可适当增大`timeoutMillis`
- 需要 ROM V12.6
...
```
请求级TTS异步接口。用于通过回调确认TTS请求是否成功，不等待播放结束。

**参数说明：**
- `text: String` - 要播放的文本
- `timeoutMillis: Long` - 请求超时时间，单位毫秒，默认3秒；不包含语音播放时长
- `callback: TTSCallback?` - 回调，可为空；`status=1`表示请求成功，`status=2`表示超时、失败或被打断

**注意：**
- `text`为空字符串时，如果传入了`callback`，SDK会立即回调`status=2`
- 需要 ROM V12.6

**与现有接口的区别：** `tts()`、`ttsSync()`等待完整播放结束，默认超时180秒；`ttsRequest()`、`ttsRequestSync()`用于确认请求是否成功，不等待播放结束，默认超时3秒。
...
```
设置设备的全局语音交互模式，与设备设置页中的语音模式切换等效。

**参数说明：**
- `mode: VoiceMode` - 目标语音交互模式，可选智能拾音、持续拾音或唤醒词模式
...
TTS任务回调接口，继承自ITaskCallback<String>。回调时机和状态含义取决于调用的TTS接口：

- 用于`tts()`、`ttsParallel()`时，播放结束后回调；`status=1`表示播放成功，`status=2`表示播放失败或被打断
- 用于`ttsRequest()`时，请求状态确定后回调，不等待播放结束；`status=1`表示请求成功，`status=2`表示请求超时、失败或被打断
- 用于`ttsRequest()`时，`result`当前为`null`
...
语音交互模式枚举，用于设置和表示设备当前的全局语音交互模式。

**包路径：** `com.ainirobot.agent.VoiceMode`

**枚举值定义：**

```kotlin
enum class VoiceMode(val value: Int) {
    WAKEUP_FREE(0),
    CONTINUOUS(1),
    WAKE_WORD(2)
}
...
```kotlin
companion object {
    fun fromValue(value: Int?): VoiceMode?
}
```

根据数值查找对应模式。`0`、`1`、`2`分别返回对应的`VoiceMode`；传入`null`或其他数值时返回`null`。

```kotlin
val mode = VoiceMode.fromValue(1) // VoiceMode.CONTINUOUS
```
