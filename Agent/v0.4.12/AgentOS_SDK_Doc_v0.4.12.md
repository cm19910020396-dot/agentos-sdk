...
- [4. 核心API接口](#4-核心api接口)
  ...
  - [4.16 语音交互模式设置与查询](#416-语音交互模式设置与查询)
- [5. 进阶功能](#5-进阶功能)
...
```Groovy
dependencies {
        implementation 'com.orionstar.agent:sdk:0.4.12-SNAPSHOT' // 【重要配置】Agent SDK依赖
...
当基础的三项配置无法满足复杂需求时，可以结合使用**感知信息上报**功能（详见[4.10节](#410-感知信息上报)）进行更灵活的信息传递：
...
v0.4.12新增请求级TTS接口，用于在指定时间内确认TTS请求是否成功，无需等待语音播放完成。该接口适用于后续业务不依赖播放完成状态的场景；如需在播放完成后执行后续逻辑，请使用`ttsSync()`或`tts()`。
 ...
 *
 * @param text 要播放的文本
 * @param timeoutMillis 请求超时时间，单位毫秒，默认3000ms
 * @return status=1表示请求成功；status=2表示超时、失败或被打断
 */
...
/**
 * 请求级TTS异步接口，结果通过TTSCallback返回
 *
 * @param text 要播放的文本
 * @param timeoutMillis 请求超时时间，单位毫秒，默认3000ms
 * @param callback status=1表示请求成功；status=2表示超时、失败或被打断
 */
fun ttsRequest(
    text: String,
    timeoutMillis: Long = 3000,
...
// 同步调用：status=1只表示请求成功，不表示播放已经完成
fun requestTtsSync() {
    AOCoroutineScope.launch {
...
| 对比项 | `tts()` / `ttsSync()` | `ttsRequest()` / `ttsRequestSync()` |
|---|---|---|
| 回调/返回时机 | 完整播放结束 | TTS请求成功或失败时，不等待播放结束 |
| 超时范围 | 从调用开始至播放结束 | 从调用开始至返回或回调，不包含语音播放时长 |
| 默认超时 | 180000ms | 3000ms |
| 适用场景 | 后续流程依赖播放完成 | 后续流程只需确认请求是否成功 |

> **注意事项**
> - `status=1`只表示请求成功，不能据此判断播放完成
> - `status=2`表示请求超时、失败或被打断，不区分具体原因
> - `result`目前为`null`，不包含额外结果
> - `timeoutMillis`不包含语音播放时长；网络状况较差时可适当调大
> - `status=2`时，该次请求已终止，不会继续播放
> - `text`为空字符串时，同步接口返回`status=2`；异步接口在提供回调时回调`status=2`
> - 需要 ROM V12.6
...
/**
 * @param text 要播放的文本
 * @param timeoutMillis 超时时间，单位毫秒，针对单段文本，默认180秒
 * @param callback 回调，status=1表示播放成功，status=2表示播放失败
 */
fun ttsParallel(
    text: String,
...
```kotlin
val segments = listOf("第一段播报内容。", "第二段播报内容。", "第三段播报内容。")

segments.forEach { text ->
    AgentCore.ttsParallel(text = text)
}
```
...
唤醒词模式是AgentOS SDK v0.4.11新增的语音交互控制功能，提供更灵活的拾音触发机制。通过三个核心接口，开发者可以精确控制基于唤醒词的语音交互行为，适配不同场景下的交互需求。
...
v0.4.12新增语音交互模式设置与查询接口，可用于切换设备的全局语音交互模式或查询当前模式。模式设置与设备设置页中的操作等效，并对设备全局生效。
...
| 枚举值 | 数值 | 说明 |
|---|---:|---|
| `VoiceMode.WAKEUP_FREE` | 0 | 智能拾音（免唤醒模式） |
| `VoiceMode.CONTINUOUS` | 1 | 持续拾音 |
| `VoiceMode.WAKE_WORD` | 2 | 唤醒词模式，需要先说唤醒词 |
...
查询设备当前的语音交互模式。查询失败或设备返回未知值时返回`null`。
...
// 查询为同步调用，建议放在协程或后台线程中执行
AOCoroutineScope.launch {
    when (val mode = AgentCore.getVoiceMode()) {
        VoiceMode.WAKEUP_FREE -> println("当前为智能拾音模式")
        VoiceMode.CONTINUOUS -> println("当前为持续拾音模式")
        VoiceMode.WAKE_WORD -> println("当前为唤醒词模式")
        null -> println("语音交互模式查询失败")
    }
}
```

> **注意事项**
> - `setVoiceMode()`仅发起模式设置，不提供结果回调
> - 如需确认当前模式，请调用`getVoiceMode()`查询
> - `getVoiceMode()`查询失败或返回未知模式值时结果为`null`，业务代码需要处理空值
> - `getVoiceMode()`为同步查询，建议在协程或后台线程执行，避免阻塞UI线程
> - 模式设置与设备设置页操作等效，并对设备全局生效
> - 需要 ROM V12.6
