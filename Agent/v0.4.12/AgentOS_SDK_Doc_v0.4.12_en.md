# AgentOS SDK for APK V0.4.12 Documentation

## 📋 Version Information

![SDK](https://img.shields.io/badge/SDK_Version-v0.4.12-blue)
...
- [2. Action Details](#2-action-details)
  ...
  - [2.3 Action Execution Flow](#23-action-execution-flow)
    ...
    - [2.3.2 Execution Result Notification](#232-action-execution-completion-notification)
...
- [4. Core API Interfaces](#4-core-api-interfaces)
  ...
  - [4.6 TTS Speech Synthesis](#46-tts-speech-synthesis)
    - [ttsRequestSync() / ttsRequest()](#ttsrequestsync--ttsrequest)
  - [4.7 TTS Audio File Playback](#47-tts-audio-file-playback)
  - [4.8 Large Language Model Interface](#48-large-language-model-llm-interface)
  ...
  - [4.16 Voice Interaction Mode Management](#416-voice-interaction-mode-management)
...
**System Requirements**
- **AgentOS Product Version**: V12.6
- **RobotAPI Version**: 12.2.1
...
```Groovy
dependencies {
        implementation 'com.orionstar.agent:sdk:0.4.12-SNAPSHOT' // IMPORTANT: Agent SDK dependency
...
> **Key Reminder**: After any Action execution completes, you must call the `action.notify()` method to notify the system of the execution status. For detailed information, please refer to [2.3.2 Execution Result Notification](#232-action-execution-completion-notification).
...
> **🔍 Troubleshooting**: If experiencing voice interaction no response issues, please refer to [FAQ - Voice Interaction No Response](../../FAQ.md#语音交互无响应) for self-checking
...
### ttsRequestSync() / ttsRequest()

v0.4.12 adds request-level TTS APIs for confirming whether a TTS request succeeds within the specified timeout, without waiting for audio playback to finish. Use these APIs when subsequent logic does not depend on playback completion; otherwise, use `ttsSync()` or `tts()`.
...
/**
 * Request-level synchronous TTS API; call it from a coroutine
 *
 * @param text Text to play
 * @param timeoutMillis Request timeout in milliseconds; default is 3000ms
 * @return status=1 if the request succeeded; status=2 if timed out, failed, or interrupted
 */
suspend fun ttsRequestSync(
    text: String,
...
/**
 * Request-level asynchronous TTS API; result is delivered through TTSCallback
 *
 * @param text Text to play
 * @param timeoutMillis Request timeout in milliseconds; default is 3000ms
 * @param callback status=1 if the request succeeded; status=2 if timed out, failed, or interrupted
 */
fun ttsRequest(
    text: String,
    timeoutMillis: Long = 3000,
...
// status=1 only means the request succeeded; playback may still be in progress.
fun requestTtsSync() {
    AOCoroutineScope.launch {
        val taskResult = AgentCore.ttsRequestSync(
            text = "Checking that for you. Please wait.",
            timeoutMillis = 3000
        )
        if (taskResult.isSuccess) {
            // The TTS request succeeded.
        } else {
            // The request timed out, failed, or was interrupted.
        }
    }
}

// Asynchronous alternative; choose either this or the synchronous example above.
fun requestTtsAsync() {
    AgentCore.ttsRequest(
        text = "Checking that for you. Please wait.",
        timeoutMillis = 3000,
        callback = object : TTSCallback {
            override fun onTaskEnd(status: Int, result: String?) {
                if (status == 1) {
                    // The request succeeded; playback has not necessarily finished.
                } else {
                    // Timed out, failed, or interrupted.
                }
            }
        }
    )
...
**Comparison with Existing TTS APIs**

| Item | `tts()` / `ttsSync()` | `ttsRequest()` / `ttsRequestSync()` |
|---|---|---|
| Callback/return timing | After complete playback | When the TTS request succeeds or fails, without waiting for playback to finish |
| Timeout scope | From invocation until playback ends | From invocation until return or callback; excludes audio playback duration |
| Default timeout | 180000ms | 3000ms |
| Use case | The workflow depends on playback completion | The workflow only needs to know whether the request succeeded |

> **Notes**
> - `status=1` only means the request succeeded; it does not indicate playback completion
> - `status=2` indicates timeout, failure, or interruption; specific reasons are not distinguished
> - `result` is currently `null` and contains no additional result data
> - `timeoutMillis` excludes audio playback duration; increase it when network conditions are poor
> - When `status=2`, the request has ended and will not continue playing
> - For an empty `text` string, the synchronous API returns `status=2`; the asynchronous API reports `status=2` when a callback is provided
> - Requires ROM V12.6
...
```Kotlin
import com.ainirobot.agent.AgentCore
import com.ainirobot.agent.TTSAudioFileCallback
...
> **💡 Usage Guide**: If you're unsure about LLM interface use cases, it's recommended to first check the FAQ document [What to do when persona and PageAgent cannot meet business requirements](../../FAQ.md#人设和pageagent都无法满足业务需求怎么办), which includes detailed usage examples and best practices.
...
Wake word mode is a new voice interaction control feature in AgentOS SDK v0.4.11, providing more flexible listening trigger mechanisms. Through three core interfaces, developers can precisely control wake word-based voice interaction behavior, adapting to interaction requirements in different scenarios.
...
## 4.16 Voice Interaction Mode Management

### Feature Introduction

v0.4.12 adds APIs for setting and querying the device's global voice interaction mode. Mode settings are equivalent to changes made on the device settings page and take effect globally.

### Available Modes

```kotlin
import com.ainirobot.agent.VoiceMode
```

| Enum Value | Value | Description |
|---|---:|---|
| `VoiceMode.WAKEUP_FREE` | 0 | Smart listening (wake-up-free mode) |
| `VoiceMode.CONTINUOUS` | 1 | Continuous listening |
| `VoiceMode.WAKE_WORD` | 2 | Wake-word mode; the wake word must be spoken first |

### setVoiceMode()

Set the device's global voice interaction mode.
...
AgentCore.setVoiceMode(VoiceMode.CONTINUOUS)
```

### getVoiceMode()

Query the device's current voice interaction mode. It returns `null` when the query fails or the device returns an unknown value.
...
// This is a synchronous query; run it in a coroutine or background thread.
AOCoroutineScope.launch {
    when (val mode = AgentCore.getVoiceMode()) {
        VoiceMode.WAKEUP_FREE -> println("Smart listening mode")
        VoiceMode.CONTINUOUS -> println("Continuous listening mode")
        VoiceMode.WAKE_WORD -> println("Wake-word mode")
        null -> println("Failed to query the voice interaction mode")
    }
}
```

> **Notes**
> - `setVoiceMode()` initiates a mode change and does not provide a result callback
> - To confirm the current mode, call `getVoiceMode()`
> - `getVoiceMode()` returns `null` when the query fails or an unknown mode is returned; handle the nullable result
> - `getVoiceMode()` is synchronous; run it in a coroutine or background thread to avoid blocking the UI thread
> - Mode settings are equivalent to changes made on the device settings page and take effect globally
> - Requires ROM V12.6
