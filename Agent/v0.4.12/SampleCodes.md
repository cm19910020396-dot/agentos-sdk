...
`ttsRequestSync()`和`ttsRequest()`用于确认TTS请求是否成功，不等待播放完成。`status=1`只表示请求成功；需要等待播放结束时请使用`ttsSync()`或`tts()`。
...
> `timeoutMillis`不包含语音播放时长，默认3000ms；网络状况较差时可适当调大。需要 ROM V12.6。

#### 6.2 v0.4.12语音交互模式设置与查询
...
fun switchToContinuousMode() {
    // 设置为持续拾音模式。设置对设备全局生效，不提供结果回调。
    AgentCore.setVoiceMode(VoiceMode.CONTINUOUS)
}

// 查询当前模式。getVoiceMode()为同步调用，建议在协程或后台线程执行。
fun refreshVoiceMode() {
    AOCoroutineScope.launch {
        when (val mode = AgentCore.getVoiceMode()) {
            VoiceMode.WAKEUP_FREE -> println("智能拾音（免唤醒）")
            VoiceMode.CONTINUOUS -> println("持续拾音")
            VoiceMode.WAKE_WORD -> println("唤醒词模式")
...
8. **v0.4.12新增接口** - 请求级TTS、语音交互模式设置与查询

原有业务代码来自实际项目；v0.4.12新增示例展示请求级TTS及语音交互模式接口的使用方式。
...
#### 6.1 v0.4.12请求级TTS接口（Java）

以下为Java异步调用示例。回调`status=1`只表示请求成功，不表示播放完成。

```java
import com.ainirobot.agent.AgentCore;
import com.ainirobot.agent.TTSCallback;

private void requestTtsAsync() {
    AgentCore.INSTANCE.ttsRequest(
        "正在为您查询，请稍候",
        3000L,
        new TTSCallback() {
            @Override
            public void onTaskEnd(int status, String result) {
                if (status == 1) {
                    // 请求成功，不代表播放已经结束
                } else {
                    // 超时、失败或被打断；当前不能进一步区分原因
...
#### 6.2 v0.4.12语音交互模式设置与查询（Java）

```java
import com.ainirobot.agent.AgentCore;
import com.ainirobot.agent.VoiceMode;

private void switchToContinuousMode() {
    // 设置为持续拾音模式。设置对设备全局生效，不提供结果回调。
    AgentCore.INSTANCE.setVoiceMode(VoiceMode.CONTINUOUS);
}

// 查询当前模式。getVoiceMode()为同步调用，建议在后台线程执行。
private void refreshVoiceMode() {
    AsyncTaskHelper.execute(() -> {
        VoiceMode currentMode = AgentCore.INSTANCE.getVoiceMode();
        if (currentMode == null) {
            // 查询失败
        } else if (currentMode == VoiceMode.WAKEUP_FREE) {
            // 智能拾音（免唤醒）
        } else if (currentMode == VoiceMode.CONTINUOUS) {
            // 持续拾音
        } else if (currentMode == VoiceMode.WAKE_WORD) {
            // 唤醒词模式
        }
    });
}
...
8. **v0.4.12新增接口** - 请求级TTS异步调用、语音交互模式设置与查询

原有Java业务代码摘录自AgentRoleJava项目；v0.4.12新增示例展示请求级TTS及语音交互模式接口的Java调用方式。
